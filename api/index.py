...
"""
import sys, os
# Tambahkan root vercel-version ke path agar bisa import lib/
_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, _root)

# Import auth dari lib/ vercel-version (sudah ada Flask request context)
# lib/auth.py vercel-version menggunakan flask.request via app context
...
def auth_register():
    return _auth.register(sb_get, sb_post)

@app.route("/api/auth/login", methods=["POST"])
def auth_login():
    return _auth.login(sb_get, sb_post)

@app.route("/api/auth/logout", methods=["POST"])
def auth_logout():
    return _auth.logout(sb_get, sb_delete)

@app.route("/api/auth/me", methods=["GET"])
def auth_me():
    return _auth.me(sb_get)

@app.route("/api/auth/members", methods=["GET"])
def auth_list_members():
    return _auth.list_members(sb_get)

@app.route("/api/auth/members", methods=["POST"])
def auth_add_member():
    return _auth.add_member(sb_get, sb_post)

@app.route("/api/auth/change-password", methods=["POST"])
def auth_change_password():
    return _auth.change_password(sb_get, sb_patch)

@app.route("/api/auth/request-reset", methods=["POST"])
def auth_request_reset():
    return _auth.request_reset(sb_get, sb_post)

@app.route("/api/auth/reset-password", methods=["POST"])
def auth_reset_password():
    return _auth.do_reset(sb_get, sb_post, sb_patch)

@app.route("/api/auth/link-telegram", methods=["POST"])
def auth_link_telegram():
    return _auth.link_telegram(sb_get, sb_patch)

@app.route("/api/auth/invite", methods=["POST"])
def auth_generate_invite():
    return _auth.generate_invite(sb_get, sb_post)

@app.route("/api/auth/join", methods=["POST"])
def auth_join_invite():
    return _auth.join_via_invite(sb_get, sb_patch, sb_post)
...
def get_transactions():
    ...
    try:
        ...
        return jsonify({"error": "Koneksi database bermasalah, coba lagi"}), 503
...
def add_transaction():
    ...
    try:
        res = sb_post("transactions", {
            "family_id":data.get("family_id", family_id),"type":data["type"],
            "amount":amount,"description":data["desc"].strip(),
            "category":data["category"],"member":data["member"],
...
def delete_transaction(tx_id):
    ...
    if not user: return jsonify({"error": "Tidak terautentikasi"}), 401
    try:
        sb_delete("transactions", {"id":f"eq.{tx_id}","family_id":f"eq.{family_id}"})
...
def reset_transactions():
    ...
    try:
        ...
        if all_txs:
            ids = [str(t["id"]) for t in all_txs]
            sb_delete("transactions",{"id":f"in.({','.join(ids)})"})
...
def get_categories():
    ...
    if not user: return jsonify({"error": "Tidak terautentikasi"}), 401
    try:
        return jsonify(sb_get("categories",[("select","*"),("order","name"),("family_id",f"eq.{family_id}")]))
...
def add_category():
    ...
    if user.get("role") != "admin": return jsonify({"error": "Hanya admin"}), 403
    data = request.json
    if not data or not data.get("name"): return jsonify({"error": "Nama wajib diisi"}), 400
    try:
...
def summary():
    ...
    try:
        y,m=month.split("-"); last=calendar.monthrange(int(y),int(m))[1]
        txs = sb_get("transactions",[("select","type,amount,category"),
...
def summary_per_member():
    ...
    for t in txs:
        m=t["member"]
        if m not in by_member: by_member[m]={"income":0,"expense":0}
        if t["type"]=="inc": by_member[m]["income"]+=t["amount"]
        else: by_member[m]["expense"]+=t["amount"]
...
def get_settings():
    ...
    try:
        ...
        return jsonify({"settings":cfg,"members":mems})
    except Exception as e:
        return jsonify({"error": "Koneksi database bermasalah, coba lagi"}), 503
...
def priv_add():
    ...
    try:
        res = sb_post("private_notes",{"family_id":family_id,"owner_id":user["id"],"type":data["type"],
            "amount":int(data["amount"]),"note_desc":str(data["desc"]).strip(),
            "category":data["category"],"date":data.get("date",today()),"source":data.get("source","web")})
        return jsonify(res[0] if isinstance(res,list) else res), 201
    except Exception as e:
        return jsonify({"error": "Koneksi database bermasalah, coba lagi"}), 503
...
def priv_delete(note_id):
    ...
    if not user: return jsonify({"error": "Tidak terautentikasi"}), 401
    try:
        sb_delete("private_notes",{"id":f"eq.{note_id}","owner_id":f"eq.{user['id']}"})
...
# ── Register webhook saat startup ────────────────────────────────────────────
def _setup_webhook():
    TELEGRAM_TOKEN = os.environ.get("TELEGRAM_TOKEN","")
    WEBHOOK_SECRET = os.environ.get("TELEGRAM_WEBHOOK_SECRET","")
    PUBLIC_URL     = os.environ.get("VERCEL_URL") or os.environ.get("PUBLIC_URL","")
    if not TELEGRAM_TOKEN or not PUBLIC_URL:
        print("[Bot] TELEGRAM_TOKEN atau PUBLIC_URL kosong — webhook dilewati")
        return
    ...
    try:
        ...
        sys.path.insert(0, bot_dir)
        import bot as tg_bot
        webhook_url = PUBLIC_URL.rstrip("/") + "/api/telegram/webhook"
        payload = {"url": webhook_url}
        if WEBHOOK_SECRET: payload["secret_token"] = WEBHOOK_SECRET
        tg_bot.tg("setWebhook", **payload)
        tg_bot.setup_commands()
        print(f"[Bot] Webhook aktif: {webhook_url}")
    except Exception as e:
        print(f"[Bot] Gagal setup webhook: {e}")
